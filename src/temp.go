...
var platform[3] Rect
var ladder[3] Rect
func on_platform(player Rect) int{
  var i int
...
func alligned_with_ladder(player Rect) int{
  var i int
...
func set_according_to_ladder_top(player Rect) Rect{ // originally was alligned but not not alligned
	var i int
	var center int=(player.XLo+player.XHi)/2
	  for i=0;i<len(ladder);i++{
	    if ladder[i].YLo>=player.YLo && ladder[i].YHi<=player.YLo {
...
func set_according_to_ladder_botton(player Rect) Rect{
	var i int
	var center int=(player.XLo+player.XHi)/2
	  for i=0;i<len(ladder);i++{
	    if ladder[i].YLo>=player.YLo && ladder[i].YHi<=player.YLo {
...
func falls_from_block(player Rect) Rect{ // originally was on platform but not now
...
func collides_with_block_vertically(player Rect) int{ // falling vertically
  var i int
  for i=0;i<len(platform);i++{
  //	fmt.Println(player.YLo,platform[i].YHi,player.XHi,platform[i].XLo , player.XLo,platform[i].XHi)
...
func collides_with_block_on_right_move(player Rect) int{
	var i int
	for i=0;i<len(platform);i++{
...
func collides_with_block_on_left_move(player Rect) int{
	var i int
	for i=0;i<len(platform);i++{
...
func get_position_collides_with_block_on_right(player Rect)Rect{
	var i int
	for i=0;i<len(platform);i++{
		if player.XLo>platform[i].XHi && player.YLo>platform[i].YHi && player.YHi<platform[i].YLo{
...
func get_position_collides_with_block_ver(player Rect)Rect{
	var i int
  for i=0;i<len(platform);i++{
      if(player.YLo>platform[i].YHi && player.XLo>platform[i].XHi && player.XHi<platform[i].XLo){ // foot of player collides with top of block
...
func GetPositino(rect player) Position{
	var temp Position
	temp.X=(player.XHi+player.XLo)/2
	temp.Y=(player.YHi+player.YLo)/2
	return temp
}
// func ValidateUp(Newposition Rect,Originlposition Rect) Rect
// {
// 	if !alligned_with_ladder(Newposition) && check_height(Newposition) {
// 		return Originlposition
// 	} else if(alligned_with_ladder(Newposition) && !check_height(Newposition))
// 		return 
// 	 else {
// 		return 
// 	}
// }
// func ValidateDown(Newposition Rect,Originlposition Rect) Rect{ // ensure not on platform
// 	if alligned_with_ladder(Newposition) && check_height(Newposition){
// 		return Newposition
// 	} else if alligned_with_ladder(Newposition) && !	check_height(Newposition){
// 		return  RestrictedPositionDown(Newposition)
// 	}else{
// 		return Originlposition
// 	}
// }
func main() {
	...
	fmt.Println(collides_with_block_vertically(p11))
